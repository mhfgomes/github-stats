...
- `/api/stats` discovers commits across all branches and recent PR heads (same attribution rules as before), batches diff stats via GraphQL with REST fallback, and caches responses in-memory for 2 minutes.
